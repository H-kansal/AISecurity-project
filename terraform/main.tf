terraform {
  ...
  backend "s3" {                                                       # This is the S3 bucket where Terraform stores the notebook.
    bucket         = "research-agent-tfstate-0106"
    key            = "terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "research-agent-tf-locks-0106"                            # This is the DynamoDB table used for locking the Terraform state file so that two instances of Terraform do not overwrite each other's changes.
    encrypt        = true
  }
}
...
variable "acm_certificate_arn" {
  ...
  default     = "" 
}             #If you provide an ACM certificate ARN, your load balancer can serve HTTPS. If it's left empty (""), the application will use HTTP only.

# ─── Data & Locals ────────────────────────────────────────────────────────────

data "aws_availability_zones" "available" {} #This data source retrieves information about available availability zones in your AWS region. The `slice` function then selects the first two AZs for deploying resources, ensuring high availability.

locals {
  azs          = slice(data.aws_availability_zones.available.names, 0, 2)
  https_enabled = var.acm_certificate_arn != ""         #  This local variable is set to true if an ACM certificate ARN is provided, and false otherwise. It is used to conditionally enable HTTPS.
}
...
# in private network we haven't any internet so if any resource want to talk to another it needed a bridge that is the these Endpoints

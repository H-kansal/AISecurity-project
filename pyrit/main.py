...
pyrit_initialized = False


async def init_pyrit():    # this will intialize pyrit 
    global pyrit_initialized
    if not pyrit_initialized:
        await initialize_pyrit_async(memory_db_type=SQLITE)    # we need a db to store the attack history
        pyrit_initialized = True


async def _getredis() -> aioredis.Redis | None:
    global redis
...
async def _run_attack_type(attack_type: str, prompts: list[str], base_risk: int) -> list[dict]:
    ...
    for i, prompt in enumerate(prompts):
        ...
        # as we find the crescendo attack is detected we stop next prompts
...
async def get_results():
    return {"results": await _load_results(), "running": running}
